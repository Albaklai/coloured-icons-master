...
Before you publish a new release, make sure the generated CSS is up to date and the version number reflects your changes. The general workflow is:

1. Update icons or other assets in `public/logos`.
2. Run the generator and build steps:
   ```bash
   pnpm run dev      # regenerates logos.css and related files locally
   pnpm run build    # run again to verify the build output
   pnpm run minify   # optional - produces the minified stylesheet used on the CDN
   ```
3. Bump the `version` field in `package.json` (e.g. `1.9.8`).
4. Commit your changes and push to GitHub.
5. Create a git tag matching the version (e.g. `git tag v1.9.8 && git push --tags`).
6. Open a release on GitHub or rely on jsDelivr to serve via the new tag; the CDN will immediately make the files available at `https://cdn.jsdelivr.net/gh/dheereshag/coloured-icons@<version>/app/ci.min.css`.

Once the tag is pushed, the published CSS can be referenced by projects using the versioned jsDelivr URL above. If you need the `@master` latest build, simply push to the `main`/`master` branch and it will be served automatically.
